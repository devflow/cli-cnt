...
const updater = {
  common: () => {
    ...
    pipe.exec((_, results) => {
      ...
      resultChunk.total = parseInt(resultChunk.current_win) + parseInt(resultChunk.current_darwin);
      resultChunk.updated = new Date();
    });
  },

  updateDayListData: () => {
    const dayList = [], now = moment();
    const pipe = redis.pipeline();
...
function addLog(msg) {
  logging.unshift(`<span class="time">${(new Date()).toLocaleString("ko-KR", {timeZone: "Asia/Seoul"})}</span>: ${msg}`);
